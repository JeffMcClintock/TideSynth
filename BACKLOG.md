...
| ID | Status | Plat | Item |
|---|---|---|---|
| P4a | BLOCKED | win | **Fix the resize crash in `gmpi_ui` — the host-killer.** `DrawingFrame::reSize` (`gmpi_ui/backends/DrawingFrameWin.cpp:1401-1430`) checks `d2dDeviceContext` at line 1406, calls `SetWindowPos` at 1408 — which dispatches `WM_SIZE` **synchronously** into the same object's `WindowProc` (`:592-597`) → `OnSize` (`:1371`) → failed `ResizeBuffers` → `ReleaseDevice()` (`DrawingFrameWin.h:376-377`) nulls both pointers — then dereferences `d2dDeviceContext` at 1418 and takes the DAW down. Re-check `d2dDeviceContext` **and** `swapChain` after `SetWindowPos` returns, exactly as the sibling `OnSize` already does at `:1376`; line 1419 never checks `swapChain` at all and is a second latent null deref. Full evidence in [docs/p4-resize-crash.md](docs/p4-resize-crash.md). Blocked only by **G3** — `gmpi_ui` is not in the run prompt's ALLOWED or GATED list. |
| P4b | BLOCKED | win | **Clamp the requested editor size in the VST3 wrapper.** REAPER asked for `{0,0,2178,32672}`; 32672 exceeds the D3D11 maximum texture dimension (16384), so `ResizeBuffers` can never succeed and the editor could not resize even with P4a fixed. `SEVSTGUIEditorWin::onSize` (`GMPI_Wrappers/wrapper/VST3/SEVSTGUIEditorWin.cpp:85-89`) forwards the host's rect with no validation, and `checkSizeConstraint` (`:119-133`) never adjusts the rect it is handed — the VST3 contract says it should return the nearest acceptable size. Where the bogus rect originates is **not** established; see the loose end in [docs/p4-resize-crash.md](docs/p4-resize-crash.md). Blocked only by **G3**. |

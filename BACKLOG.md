...
| S4 | DOING | any | *(claimed 2026-08-07, linux, branch `tide/linux/S4-sem-cache-clobber`)* **TIDE can clobber the installed SynthEdit's module cache.** `TideApp` sets `BundleInfo::semFolder` but not `isSemFolderOverridden` (`SynthEditLib/modules/se_sdk3_hosting/BundleInfo.h:63`), so `SemCacheName()` (`SE16/SynthEdit2/ModuleFactory_Editor.cpp:174`) omits its `-override-<hash>` suffix and TIDE reads **and writes** the same `Plugin-Cache-16.xml` as the desktop SynthEdit app. A TIDE instance in a DAW can overwrite the desktop app's cache with TIDE's smaller module set. Live Windows/macOS hazard today, independent of iOS. Subsumed by S1a if S1a lands first — close it then rather than fixing twice. |
